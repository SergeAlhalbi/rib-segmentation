...
def unet2_5_D(sz, depth=3, filts=16, num_classes=2, drop_rate=0.5, 
    ...
    for i in range(depth):  #FYI, for some, batchnorm in-between conv and relu (paper)
        net = Conv2D(filts*2**i, 3, kernel_initializer='he_normal', padding='same', 
                     name='Conv2_5_D_Enc_'+str(i)+'_1') (net)
        if batch_norm_bool:
        ...
        net = Conv2D(filts*2**i, 3, kernel_initializer='he_normal', padding='same',
                     name='Conv2_5_D_Enc_'+str(i)+'_2') (net)
        if batch_norm_bool:
    ...
    i = depth;
    net = Conv2D(filts*2**i, 3, kernel_initializer='he_normal', padding='same',
                 name='Conv2_5_D_Bridge_'+str(i)+'_1') (net)
    if batch_norm_bool:
        net = BatchNormalization(axis=last_ax) (net)
    net = ReLU() (net)
    
    net = Conv2D(filts*2**i, 3, kernel_initializer='he_normal', padding='same',
                 name='Conv2_5_D_Bridge_'+str(i)+'_2') (net)
    if batch_norm_bool:
        net = BatchNormalization(axis=last_ax) (net)
    net = ReLU() (net)
    ...
    for i in reversed(range(depth)):
        net = Conv2DTranspose(filts*2**i, 3, strides=(2, 2), padding='same',
                              name='Conv2_5_D_Trans_'+str(i)) (net)
        net = concatenate([net, c[i]], axis=last_ax)
        
        net = Conv2D(filts*2**i, 3, kernel_initializer='he_normal', padding='same',
                     name='Conv2_5_D_ec_'+str(i)+'_1') (net)
        if batch_norm_bool:
            net = BatchNormalization(axis=last_ax)(net)
        net = ReLU() (net)
        
        net = Conv2D(filts*2**i, 3, kernel_initializer='he_normal', padding='same',
                     name='Conv2_5_D_ec_'+str(i)+'_2') (net)
        if batch_norm_bool:
            net = BatchNormalization(axis=last_ax)(net)
        net = ReLU() (net)
    ...
    outputs = Conv2D(num_classes, 1, activation=final_activation, 
                     ...
                     name='Conv2_5_D_Final') (net) #<-adding padding same explicitly
...
def process_mat_train(input_name, aug_bool=True,
    ...
    if window_rng_bool:
        ...
        # Consecutive slices processing
    if consec_dif_bool:
        slices = im_input.shape[2] # assumes 2.5D
        mid = round( (slices-1) / 2)
        
        im_input = tf.transpose(im_input, perm=[2,0,1])
        mid_slice = im_input[mid, ...]
        
        for ind in range(slices):
            if ind != mid:
                indices = [[ind]]
                updates = im_input[ind, ...] - mid_slice
                updates = updates[tf.newaxis, ...]
                
                im_input = tf.tensor_scatter_nd_update(im_input, indices, updates)
                # im_input[..., ind] = im_input[..., ind] - mid_slice
                
        im_input = tf.transpose(im_input, perm=[1,2,0])
        del slices, mid, mid_slice, ind, indices, updates
...
def viewTest(model, test_files, arg_list, cur_i=None, cur_z=None, miscTxt=None):
    ...
    print('Image shape:',cur_im.shape)
    print('Image type:',cur_im.dtype)
    print('Image values:',np.unique(cur_im))
    
    print('Target shape:',cur_tar.shape)
    print('Target type:',cur_tar.dtype)
    print('Target values:',np.unique(cur_tar))
...
if __name__ == '__main__':
    ...
    # DATA PATH
    data_paths = [
        r'D:\Serge 231\Rib\Data\Rib_Patches_3Slices'
                 ]
    
    name_prefix = '20230418_512_2_5_RibSeg_'
    ...
    for model_n in range(len(models_list)):
        ...
        # Generate new model
        model = unet2_5_D(sz, depth, filters, num_classes, \
        ...
        out_folder = r'D:\Serge 231\Rib\Models\Rib_Models_2_5_D'
        ...
        arg_list = build_arg_list(num_classes=num_classes,
                                  consec_dif_bool=False,
                                  window_rng_default=[-250, 500],
                                  window_rng_type='norm-clip',
                                  level_rng=[100, 150], window_rng=[825, 875],
