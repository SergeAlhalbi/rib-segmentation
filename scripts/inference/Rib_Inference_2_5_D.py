...
Created on Wed Apr 19 10:12:36 2023
...
if __name__ == '__main__':
    ...
    data_path = r'D:\Serge 231\Rib\Data\Rib_Volumes_2_5_D'
    main_save_path = r'D:\Serge 231\Rib\Data\Rib_Inference_2_5_D'
    ...
    # Load model
    model_folder_path = r'D:\Serge 231\Rib\Models\Rib_Models_2_5_D\20230418_1412--2_5-d4f16s512b16-20230418_512_2_5_RibSeg_Group1'
    ...
    for file_n in range(file_ns):
        ...
        first_slice = vol[:, :, 0]
        last_slice = vol[:, :, -1]
        vol = np.concatenate([first_slice[:,:,np.newaxis], vol, last_slice[:,:,np.newaxis]], axis=2)
        ...
        # Loop over slices
        mask_vol = np.empty((vol.shape[0], vol.shape[1], vol.shape[2]-2), dtype=np.uint8)
        for im_nb in range(vol.shape[2]-2):
            
            #  Extract
            im_input = vol[:,:,im_nb:im_nb+im_dp]
            
            # Preprocess
            im = process_mat_inference(im_input, crop_bool=False,
                                       window_rng_bool=True,
                                       window_rng_default=[-300, 550],
        ...
        # Save
        mask_dict = {"predictedRibMask_2_5_D": mask_vol}
