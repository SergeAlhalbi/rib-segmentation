...
Created on Thu Mar 30 16:43:53 2023
...
if __name__ == '__main__':
    ...
    for file_n in range(file_ns):
        ...
        # Directories
        true_path = os.path.join('D:', '\Serge 231', 'Rib', 'Data', 'Rib_Masks', folder_nm, patient_n + '.mat')
        pred_path = os.path.join('D:', '\Serge 231', 'Rib', 'Data', 'Rib_Inference', folder_nm, 'Group' + str(grp_n), 'pos', patient_n + '.mat')
        ...
        # Extract
        true_mask = true_dict['refinedRibMask']
        pred_mask = pred_dict['predictedRibMask']
    ...
    df.to_excel(os.path.join(main_save_path, 'Rib_Metrics', '2_D') + '\dice_list.xlsx', index=False)
    df_s = pd.DataFrame(sorted_patient_dice_list.T, columns=['Dataset', 'Group', 'Patient', 'Dice'])
    df_s.to_excel(os.path.join(main_save_path, 'Rib_Metrics', '2_D') + '\sorted_dice_list.xlsx', index=False)
    ...
    for file_n_r in range(first_lowest):
        ...
        # Directories
        true_path_r = os.path.join('D:', '\Serge 231', 'Rib', 'Data', 'Rib_Masks', folder_nm_r, patient_n_r + '.mat')
        pred_path_r = os.path.join('D:', '\Serge 231', 'Rib', 'Data', 'Rib_Inference', folder_nm_r, 'Group' + str(grp_n_r), 'pos', patient_n_r + '.mat')
        ...
        # Extract
        true_mask_r = true_dict_r['refinedRibMask']
        pred_mask_r = pred_dict_r['predictedRibMask']
        ...
        savemat(os.path.join(main_save_path, 'Rib_Review', '2_D', str(file_n_r + 1) + '_' + folder_nm_r + '_' + patient_n_r + '.mat'), slice_dice_dict)
